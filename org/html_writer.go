...
func (w *HTMLWriter) WriteRegularLink(l RegularLink) {
	...
	case "id":
		...
		url = strings.TrimPrefix(file, "/Users/ladicle/doc/content") // FIXME: workaround for roam-db
...
func (w *HTMLWriter) WriteNodeWithMeta(n NodeWithMeta) {
	...
	if len(n.Meta.Caption) != 0 {
		...
		if n.Meta.Name != "" {
			...
			number = fmt.Sprintf(`<span class="caption-number"><a id="%s">%s</a>:</span>`,
				l.ID(), l.DescriptionWithLang(w.document.Get("LANGUAGE")))
		}
